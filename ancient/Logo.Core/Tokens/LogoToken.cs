...
namespace Logo.Core.Tokens
{
    ...
    public class LogoToken
    {
        ...
        public virtual LogoToken Clone()
        {
            return new LogoToken { Evaluated = Evaluated, Literal = Literal, TokenValue = TokenValue };
        }
        ...
        public static TokeniserResult TokeniseString(string input)
        {
            ...
            result.TokenisedData = new List<LogoToken>();
            ...
            try
            {
                for (idx = 0; idx < input.Length; ++idx)
                {
                    ...
                    else if (inWord)
                    {
                        if (Char.IsWhiteSpace(input[idx]))
                        {
                            result.TokenisedData.Add(new LogoWord { Literal = input.Substring(startPos, idx - startPos) });
                            inWord = false;
                        }
                        else if (input[idx] == '[')
                        {
                ...
                if (inWord)
                {
                    result.TokenisedData.Add(new LogoWord { Literal = input.Substring(startPos) });
                }
